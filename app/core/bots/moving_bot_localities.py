# app/core/bots/moving_bot_localities.py
"""
Israeli locality dataset — static JSON loader + normalized lookup.

Loads ``localities.json`` at import time (same pattern as
``moving_bot_pricing.py`` loading ``pricing_config.json``) and builds
a normalized name→Locality lookup for Hebrew, English, and Russian names.

Used by the route-band classifier (``moving_bot_geo.classify_route``)
...
_LOCALITIES_PATH = Path(__file__).parent / "localities.json"
_RU_ALIASES_PATH = Path(__file__).parent / "localities_ru_aliases.auto.json"
