# app/core/bots/moving_bot_pricing.py
"""
Pricing configuration for the Moving Bot — Nationwide zone-based.

All tunable values live in ``pricing_config.json`` (same directory).
...
_CONFIG_PATH = Path(__file__).parent / "pricing_config.json"
...
def estimate_price(
    *,
    ...
    5. **Item costs** — midpoint of each item's (min, max) range × qty, summed.
